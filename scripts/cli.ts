...
async function initSubmodules(skipPrompt = false) {
  ...
  if (existingProjects.length > 0) {
    p.log.info(
      `Already initialized: ${existingProjects.map((p) => p.name).join(", ")}`
    );
  }
}
  ...
  for (const [vendorName, config] of Object.entries(vendors)) {
    ...
    for (const [sourceSkillName, outputSkillName] of Object.entries(
      ...
      if (!existsSync(sourceSkillPath)) {
        p.log.warn(
          `Skill not found: vendor/${vendorName}/skills/${sourceSkillName}`
        );
        continue;
      }
...
async function cleanup(skipPrompt = false) {
  ...
  const extraSkills = existingSkills.filter(
    (name) => !expectedSkills.has(name)
  );
